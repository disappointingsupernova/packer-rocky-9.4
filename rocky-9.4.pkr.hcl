...
source "vsphere-iso" "linux-rocky-94" {
  ...
  vm_name = "${var.vm_guest_os_family}-${var.vm_guest_os_vendor}-${var.vm_guest_os_member}-${var.vm_guest_os_version}-${local.buildtime}"
  ...
  boot_command = [
    "<esc><wait>",
    "linux inst.ks=cdrom:/ks.cfg<enter><wait><enter>"
  ]
  ip_wait_timeout = "20m"
